...
from godisalotachat.app_paths import data_dir
...
class Info3ditorPlugin(ThreadedPlugin):
    ...
    def __init__(self) -> None:
        ...
        self._install_settings_dialog_patch()
    ...
    def _install_settings_dialog_patch(self) -> None:
        if QtWidgets is None:
            return
        try:
            from godisalotachat.ui import settingsdialog as settingsdialog_module
            dialog_cls = settingsdialog_module.PluginSettingsDialog
        except Exception:
            return

        try:
            setattr(dialog_cls, '_info3ditor_plugin_instance', self)
            if getattr(dialog_cls, '_info3ditor_embedded_patch_installed', False):
                return
            original_init = dialog_cls.__init__

            def patched_init(dialog_self, plugin_name, schema, values, parent=None, *args, **kwargs):
                original_init(dialog_self, plugin_name, schema, values, parent, *args, **kwargs)
                try:
                    is_info3ditor = any(isinstance(field, dict) and field.get('key') == '__info3ditor_embedded__' for field in (schema or []))
                    if not is_info3ditor:
                        return
                    plugin = getattr(dialog_cls, '_info3ditor_plugin_instance', None)
                    if plugin is not None:
                        plugin._replace_settings_dialog_with_panel(dialog_self)
                except Exception:
                    pass

            dialog_cls.__init__ = patched_init
            setattr(dialog_cls, '_info3ditor_embedded_patch_installed', True)
        except Exception:
            pass
