...
import base64, hashlib, json, mimetypes, os, secrets, socket, struct, sys, threading, time, urllib.parse, urllib.request, urllib.error, webbrowser, tempfile, subprocess, sqlite3, shutil, ctypes, importlib.util
...
PLATFORM_ORDER = ["twitch", "tiktok", "youtube", "kick", "spotify", "meld", "obs"]
CALLBACK_PORT = 5173
MAIN_PORT = 17890
...
class WebbasedPluginHost:
    ...
    def platform_settings(self, platform: str) -> dict:
        try:
            return dict(self.state.settings().get("platforms", {}).get(platform, {}) or {})
        except Exception:
            return {}
...
class WebbasedPluginManager:
    ...
    def discover(self) -> list[dict]:
        out = []
        try:
            for folder in sorted(self.state.plugins.iterdir()):
    ...
    def ensure_started(self) -> None:
        if self.started:
            return
        self.started = True
        # Erstmal nur twitch_chat automatisch starten. Die restlichen Plugins werden danach einzeln sauber webbased angepasst.
        self.start_plugin("twitch_chat")
    ...
    def load_plugin(self, plugin_id: str):
        ...
        folder = self.state.plugins / plugin_id
        plugin_file = folder / "plugin.py"
        if not plugin_file.exists():
            raise RuntimeError(f"plugin.py fehlt: {plugin_id}")
        root = str(self.state.base)
        plugins_root = str(self.state.plugins)
        for item in (root, plugins_root, str(folder)):
...
class AppState:
    def __init__(self, base: str, port: int):
        ...
        # UI resources may be next to the script OR inside PyInstaller's _internal/_MEIPASS.
        resource_candidates = [
            self.base,
            self.base / "_internal",
            Path(getattr(sys, "_MEIPASS", self.base)),
        ]
        self.resource_base = next((p for p in resource_candidates if (p / "templates").exists() and (p / "static").exists()), self.base)
    ...
    def plugin_enabled(self, plugin_id: str) -> bool:
        try:
            ...
            # twitch_chat ist der erste webbased-portierte Chat-Reader und startet standardmäßig,
            # sobald Twitch im Plattform-Tab verbunden ist. Später können weitere Plugins exakt so folgen.
            if plugin_id == "twitch_chat" and "enabled" not in pcfg:
                return True
            return bool(pcfg.get("enabled", False))
        except Exception:
            return plugin_id == "twitch_chat"
    ...
    def plugin_settings(self, plugin_id: str, plugin=None) -> dict:
        ...
        if plugin_id == "twitch_chat":
            ...
            cfg.setdefault("viewer_count_enabled", True)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/settings":
            ...
            try:
                if "twitch" in (incoming.get("platforms", {}) if isinstance(incoming.get("platforms"), dict) else {}):
                    old_plugin = st.plugin_instances.get("twitch_chat")
                    if old_plugin is not None:
                        try: old_plugin.stop(wait=True)
                        except TypeError: old_plugin.stop()
                    st.plugin_manager.start_plugin("twitch_chat")
            except Exception as exc:
                st.log("twitch_chat", "restart after settings failed", exc)
            try:
        ...
        if path == "/api/message":
            ...
            if text:
                st.messages.append({"id": _now_ms(), "platform":"test", "user":"Test", "text":text, "time":time.strftime("%H:%M:%S")})
            return self._json({"ok": True})
    ...
    def _overlay_default(self):
        return {
            ...
            "title": {"enabled": True, "x": 136, "y": 34, "w": 270, "h": 42, "fontSize": 28, "fontFamily": "Segoe UI", "color": "#ffffff", "uppercase": True},
            "artist": {"enabled": True, "x": 137, "y": 77, "w": 270, "h": 30, "fontSize": 18, "fontFamily": "Segoe UI", "color": "#d7dcff"},
            "extras": []
        }
    ...
    def _clean_overlay_settings(self, data):
        ...
        for item in data.get("extras", cur.get("extras", [])) if isinstance(data.get("extras", cur.get("extras", [])), list) else []:
            ...
            extras.append({"id": text(item.get("id"), "x"+str(int(time.time()*1000))), "text": text(item.get("text"),"Neues Element"), "enabled": bool(item.get("enabled", True)), "x": int(num(item.get("x"),50)), "y": int(num(item.get("y"),150)), "w": int(num(item.get("w"),260,8,3000)), "h": int(num(item.get("h"),40,8,1000)), "fontSize": int(num(item.get("fontSize"),24,6,300)), "fontFamily": text(item.get("fontFamily"),"Segoe UI"), "color": text(item.get("color"),"#ffffff")})
