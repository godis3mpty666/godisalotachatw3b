...
class OpenAIChatClient:
    ...
    def _error_message(self, exc: urllib.error.HTTPError) -> str:
        ...
        if exc.code == 404:
            return "OpenAI Fehler 404: Modell nicht verfuegbar."
    ...
    def test_connection(self, settings: dict) -> tuple[bool, str]:
        ...
        try:
            ...
            return (bool(text), f"OpenAI verbunden: model={model}, response={text[:40]}" if text else "OpenAI verbunden, aber ohne Textausgabe.")
    ...
    def generate(self, settings: dict, source_platform: str, username: str, text: str, reason: str, context_text: str) -> str:
        ...
        prompt = str(settings.get("system_prompt") or "").strip() or "Du bist botalot. Antworte kurz, direkt, hilfreich und menschlich."
        instructions = (
            f"{prompt}\n\nTrigger: {reason}. Plattform: {source_platform}. Nutzer: {username}. "
            f"Antworte maximal {max_chars} Zeichen. Erwaehne Ursula nur, wenn die aktuelle Nachricht sie ausloest oder danach fragt."
        )
        input_text = "Letzte Nachrichten dieses Nutzers:\n" + (context_text or "(keine)") + f"\n\nAktuell: {text}"
        max_tokens = max(60, min(260, max_chars * 2))
        try:
            if self._is_responses_model(model):
                payload = {"model": model, "instructions": instructions, "input": input_text, "max_output_tokens": max_tokens}
                if model.lower().startswith("gpt-5"):
                    payload["reasoning"] = {"effort": "minimal"}
            ...
            self._log(self._error_message(exc))
        except Exception as exc:
