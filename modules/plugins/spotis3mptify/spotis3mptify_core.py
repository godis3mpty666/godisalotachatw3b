...
function deleteSelected(){if(!selected)return;state.elements=state.elements.filter(e=>e.id!==selected);selected=null;render()}function resetLayout(){if(confirm('Layout zurÃ¼cksetzen?'))fetch('/customoverlay/state?default=1').then(r=>r.json()).then(d=>{state=d;selected=null;nodes.clear();stage.innerHTML='';render()})}function saveLayout(){fetch('/customoverlay/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(state)}).then(r=>r.json()).then(d=>alert(d.ok?'Gespeichert':'Fehler: '+(d.error||'?'))).catch(e=>alert(e))}function loadLayout(retry=true){return fetch('/customoverlay/state?_='+Date.now(),{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('state '+r.status);return r.json()}).then(d=>{state=d;selected=null;nodes.clear();stage.innerHTML='';render()}).catch(()=>{if(retry)setTimeout(()=>loadLayout(true),1500)})}
render();refreshData();loadLayout(true);
</script></body></html>"""
