...
async function refreshMessages(){
  ...
  el.innerHTML=(m.messages||[]).filter(x=>x.message_type==="chat").map(x=>`<div class="msg">${platformBadge(x.platform)} <span class="small">${esc(x.time)}</span> · <b style="color:${userColor(x.platform,x.user)}">${esc(x.user)}</b>: ${x.html||esc(x.text)}</div>`).join("");
