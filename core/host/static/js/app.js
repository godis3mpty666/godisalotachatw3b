...
async function renderChat(){
  ...
  const viewerSettings=document.createElement("section");viewerSettings.className="card desktopSettings";viewerSettings.innerHTML=`<h3>Zuschauerzahl</h3><div class="platformForm viewerCountSettings"><label><div>Zuschauerzahl anzeigen</div><input class="viewerCountEnabled" type="checkbox" ${viewers.enabled!==false?"checked":""}></label></div>`;alertSettings.after(viewerSettings);
...
function openCommandsSettings(mount, values, defaults={}){
  const platformNames={twitch:"Twitch",tiktok:"TikTok",youtube:"YouTube",kick:"Kick"};
  const empty=()=>({id:`cmd_${Date.now()}`,enabled:false,name:"",trigger:"!",cooldown_seconds:0,sources:{twitch:true,tiktok:true,youtube:true,kick:true},chat_enabled:true,response:"",reply_same_platform:true,targets:{twitch:false,tiktok:false,youtube:false,kick:false},obs_enabled:false,obs_hotkey:"",meld_enabled:false,meld_action:""});
  ...
  const normalize=cmd=>{cmd={...empty(),...(cmd||{})};cmd.sources={twitch:true,tiktok:true,youtube:true,kick:true,...(cmd.sources||{})};cmd.targets={twitch:false,tiktok:false,youtube:false,kick:false,...(cmd.targets||{})};if(['callFunctionWithArgs:lurk-alert:triggerLurk:["{user}"]','switchScene:scn_13;evalJs:lyr_42:window.triggerLurk("{user}")'].includes(String(cmd.meld_action||"").trim()))cmd.meld_action='switchScene:scn_13;callFunctionWithArgs:lyr_42:triggerLurk:["{user}"]';if(!String(cmd.trigger||"").trim())cmd.trigger="!";return cmd};
  ...
  const placeholderLine=$(".small",$("#commandsEditor"));
  if(placeholderLine)placeholderLine.insertAdjacentHTML("afterend",`<div class="small">${L("Für deine Meld-Szene: Meld aktivieren und als Aktion switchScene:scn_13;callFunctionWithArgs:lyr_42:triggerLurk:[\"{user}\"] eintragen.","For your Meld scene: enable Meld and use switchScene:scn_13;callFunctionWithArgs:lyr_42:triggerLurk:[\"{user}\"].")}</div>`);
...
async function openPluginSettings(pluginId){
  ...
  if(pluginId==="commands"){openCommandsSettings(mount,d.values||{},d.defaults||{});return;}
  let schema=d.schema||[];
