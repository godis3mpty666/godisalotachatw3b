...
async function load(){
 ...
 el.innerHTML=(d.messages||[]).filter(m=>m.message_type==="chat").slice(-12).map(m=>`<div class="oMsg"><span class="p"><img src="/platform-icon/${esc(m.platform)}" alt="${esc(mark[m.platform]||m.platform)}"></span><b style="color:${userColor(m.platform,m.user)}">${esc(m.user)}</b>: <span class="chatText">${inlineMessage(m)}</span></div>`).join('');
}
